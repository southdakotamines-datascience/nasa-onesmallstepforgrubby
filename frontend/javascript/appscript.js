...
const earth = createSphere(
    EARTH_RADIUS,
    128,
    new THREE.MeshPhongMaterial({ map: earthTexture, bumpMap: bumpTexture, bumpScale: 3 })
);
scene.add(earth);
...
function spawnAsteroid(distance, angleTheta, anglePhi, speed, launchX, launchY, radius, density) {
    ...
    drawTrajectory(asteroid, 100000, 0.5);
}
...
function drawTrajectory(obj, steps = 100000, dt = 1) {
...
let tugger = null;
let tugActive = false;
let tugForce = 0.00; // now adjustable
...
//   Tug Force Slider
const tugForceSlider = document.getElementById("tugForce");
if (tugForceSlider) {
    tugForceSlider.addEventListener("input", () => {
        tugForce = parseFloat(tugForceSlider.value);
        document.getElementById("tugForceVal").textContent = tugForce.toFixed(3);
    });
}
...
let missileTrajectory = null;
let missileSpeed = 5;
let missilePayloadMT = 5;
...
function launchMissile(targetAsteroid, speed = 5, payloadMT = 5) {
    if (!targetAsteroid) return;

    missileSpeed = speed;
    missilePayloadMT = payloadMT;
    ...
    if (missileTrajectory) {
        scene.remove(missileTrajectory);
        missileTrajectory.geometry.dispose();
        missileTrajectory.material.dispose();
        missileTrajectory = null;
    }

    const dirToAsteroid = targetAsteroid.position.clone().normalize();
    const missileStart = dirToAsteroid.clone().multiplyScalar(EARTH_RADIUS + 2);

    missile = createMissileMesh();
    missile.position.copy(missileStart);
    scene.add(missile);

    missileTarget = targetAsteroid;
    missileLaunched = true;
}

function updateMissile(dt = 1) {
    if (!missileLaunched || !missile || !missileTarget) return;

    const direction = missileTarget.position.clone().sub(missile.position).normalize();
    missile.position.add(direction.clone().multiplyScalar(missileSpeed * dt));
    ...
    const points = [missile.position.clone(), missileTarget.position.clone()];
    const geom = new THREE.BufferGeometry().setFromPoints(points);
    missileTrajectory = new THREE.Line(geom, new THREE.LineBasicMaterial({ color: 0x00ff00 }));
    scene.add(missileTrajectory);

    const distanceToAsteroid = missile.position.distanceTo(missileTarget.position);
    if (distanceToAsteroid <= missileTarget.userData.radius) {
        console.log(`Missile hit asteroid! Payload: ${missilePayloadMT} MT`);

        const yieldJoules = missilePayloadMT * 4.184e15;
        const efficiency = 0.001;
        const ejectaVel = 10000;
        const impulse = efficiency * yieldJoules / ejectaVel;

        const asteroidMass =
            (4 / 3) * Math.PI * Math.pow(missileTarget.userData.radius * SCALE * 1000, 3) *
            missileTarget.userData.density;

        const deltaV = impulse / asteroidMass;
        const deltaVScaled = toThreeJSScale(deltaV / 1000);

        missileTarget.userData.velocity.add(direction.clone().multiplyScalar(deltaVScaled));
...
function updateMissileCamera(missile, asteroid, lerpFactor = 0.1) {
    if (!missile || !asteroid) return;

    const midpoint = missile.position.clone().add(asteroid.position).multiplyScalar(0.5);
    const dist = missile.position.distanceTo(asteroid.position);
    const offset = new THREE.Vector3(dist * 0.8, dist * 0.6, dist * 1.2);
...
function act() {
    ...
    if (launched && asteroid) {
        ...
        if (asteroid.position.length() <= EARTH_RADIUS) {
            ...
            console.log(
                `Asteroid hit! Crater diameter: ${crater.diameter.toFixed(1)} m, depth: ${crater.depth.toFixed(1)} m`
            );
            createImpactZonesOnEarth(crater.diameter, localImpactPos);
        }

        if (missileLaunched && missile) {
            updateMissileCamera(missile, asteroid);
        } else if (!missileLaunched && asteroid) {
            updateChaseCamera(asteroid, 6, 0.15);
        }
    } else if (!asteroidLaunched) {
        drawTrajectory(asteroid, 100000, 0.5);
    }
